...
기존 모노리식 아키텍쳐의 한계를 극복하기 위해서 마이크로서비스 아키텍처를 채택하고 있다.  
이전에도 MSA와 유사한 SOA(Service Oriented Architecture)가 등장하였지만, 분산처리 환경에서 ESB(Enterprise Service Bus)를 사용하여 중앙에서 서비스 간 통신을 담당했다.  
SOA는 ESB의 부하 중앙 집중 문제가 발생하여 사용되지 않으며, 통신 서비스 간 독립적으로 흐름을 제어하는 MSA가 등장하게 되었고, 클라우드를 기반으로 개발 생산성을 높이기 위해 스프링 클라우드가 출시 되었다.
